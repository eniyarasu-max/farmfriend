...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    let lastErrorMsg = 'Gemini API failed to respond.';
    let replyText = null;
    ...
    for (const model of models) {
      ...
      try {
        ...
        if (response.ok && data.candidates?.[0]?.content?.parts?.[0]?.text) {
          ...
          // If model not found or deprecated, try next model in list
          if (response.status === 404) continue;
          // If authentication or other fatal error, break immediately
          break;
        }
      } catch (fetchErr) {
        clearTimeout(timeoutId);
        if (fetchErr.name === 'AbortError') {
          return {
            statusCode: 504,
            headers: { 'Access-Control-Allow-Origin': '*' },
            body: JSON.stringify({ error: 'Sprout took too long to respond — try a shorter question or check your library.' })
          };
        }
    ...
    if (!replyText) {
      return {
        statusCode: 500,
        headers: { 'Access-Control-Allow-Origin': '*' },
        body: JSON.stringify({ error: lastErrorMsg })
      };
    }
...
>>>>>>> f6861da8cd8db2a76ffc8174abc94c925873373d
